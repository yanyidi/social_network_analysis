...
search.query.covid <- "#coronavirus"    # this is what you are searching for.  replace with whatever you are interested in
...
search.results.covid <- search_tweets(search.query, 
...
save_as_csv(search.results.covid, "search-results-covid.csv") # be sure to give this an appropriate name
...
search.results.covid <- read_twitter_csv("search-results-covid.csv")

#######################################################################################
# inspecting data
# it can be useful to know how many tweets you have collected, how many users, what the time coverage is, etc

colnames(search.results.covid) # this tells you the names of the different kinds of information returned
View(search.results.covid) # this opens the data in a new tab

nrow(search.results.covid) # this is the number of tweets
length(unique(search.results.covid$screen_name)) # this is the number of users
min(search.results.covid$created_at) # the date and time of the earliest tweet
max(search.results.covid$created_at) # the date and time of the last tweet

# you can also plot the frequency of posts over time
ts_plot(search.results.covid, "5 minutes") # note that you can adjust this interval to 'smooth' the results
...
covid.rt.el <- get.retweet.network(search.results.covid) # generate network

# extract the mention network from search results
covid.mentions.el <- get.mentions.network(search.results) # generate network
...
write.csv(covid.rt.el, "covid-rt-el.csv") #be sure to give this an appropriate name
write.csv(covid.mentions.el, "covid-mentions-el.csv") #be sure to give this an appropriate name
...
covid.rt.igraph <- graph.data.frame(covid.rt.el, directed=T)
covid.mentions.igraph <- graph.data.frame(covid.mentions.el, directed=T)

plot(covid.rt.igraph) # note that the default plots settings are not helpful

plot(covid.rt.igraph,
     vertex.size=3,       #make the nodes smaller
     vertex.label=NA,     #drop the labels
     edge.arrow.size=.5)  #make the arrows smaller

plot(simplify(covid.rt.igraph), #adding simplify() removes the loops
     vertex.size=3,       
     vertex.label=NA,     
...
plot(giant.component(simplify(covid.rt.igraph)), 
     vertex.size=3,       
     vertex.label=NA,     
     edge.arrow.size=0)
...
covid.rt.igraph.filtered <- degree.filter(giant.component(covid.rt.igraph), threshold = 2, mode = "total")
...
visualize.browser(giant.component(simplify(covid.rt.igraph.filtered)))
...
sort(degree(covid.rt.igraph, mode = "out"), decreasing = T)[1:10]

# alternatively:

centrality.covid <- data.frame(screen_name = names(degree(covid.rt.igraph, mode = "out")),
                         degree.out = degree(covid.rt.igraph, mode = "out"))

View(centrality.covid) # click on the small arrow in the corner of the 'degree.out' header to sort
...
search.results1 <- read_twitter_csv("brexit_1_2_1100AM.csv")
search.results2 <- read_twitter_csv("brexit_1_2_1145AM.csv")
